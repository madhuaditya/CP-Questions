...
public class D_Divisible_Pairs {
    ...
    public static void solve() {
        ...
        for(int i = 0; i < n; i++) {
            int modY = arr[i]%y;
            int modX = arr[i]%x;
            int reqModX = (x-modX)%x;
            if(hm.containsKey(modY)) {
                ans += hm.get(modY).getOrDefault(reqModX, 0);
            } else {
                hm.put(modY, new HashMap<>());
            }
            hm.get(modY).put(modX, hm.get(modY).getOrDefault(modX, 0)+1);
        }
